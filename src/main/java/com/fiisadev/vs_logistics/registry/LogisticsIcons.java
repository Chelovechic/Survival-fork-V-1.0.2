...
public class LogisticsIcons extends AllIcons {
    public static final ResourceLocation ICON_ATLAS = VSLogistics.asResource("textures/gui/icons.png");
    ...
    public static final LogisticsIcons
            I_SUCTION = newRow(),
            I_DISCHARGE = next();

    public LogisticsIcons(int x, int y) {
        super(x, y);
        iconX = x * 16;
        iconY = y * 16;
    }

    private static LogisticsIcons next() {
        return new LogisticsIcons(++x, y);
    }

    private static LogisticsIcons newRow() {
        return new LogisticsIcons(x = 0, ++y);
    }
